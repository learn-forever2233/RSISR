...
class ChannelAttentionModified(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        self.fc1 = nn.Conv2d(in_channels, max(1, in_channels // reduction_ratio), 1, bias=False)
        self.relu = nn.ReLU()
        self.fc2 = nn.Conv2d(max(1, in_channels // reduction_ratio), in_channels, 1, bias=False)
...
class MediumScaleFeatureExtractor(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(MediumScaleFeatureExtractor, self).__init__()
        
        self.dwconv5x5 = nn.Conv2d(in_channels, in_channels, kernel_size=5, padding=2, groups=in_channels)
...
class FeatureFusionModule(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factor, wavelet_channels):
        ...
        self.medium_scale_extractor = MediumScaleFeatureExtractor(in_channels, mid_channels)
        self.large_scale_extractor = LargeScaleFeatureExtractor(in_channels, mid_channels)
        ...
        self.fusion_conv = nn.Conv2d(mid_channels * 3, out_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        medium_scale_features = self.medium_scale_extractor(x)
        ...
        modulated_medium_scale_features = self.feature_modulation(medium_scale_features, recombined_features)
        
    
        combined_features = torch.cat([modulated_large_scale_features, modulated_medium_scale_features, small_scale_features], dim=1)
