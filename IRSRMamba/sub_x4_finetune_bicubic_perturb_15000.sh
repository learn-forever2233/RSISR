...
#SBATCH --job-name=ft_IRSR_15000 # 作业名称
...
# 3. 执行训练任务
echo "开始训练：使用参数扰动的 IRSRMamba 双三次下采样训练 (基于 15000 模型)"
echo "预训练模型：/hpcfs/fhome/competitor01/lhx/IRSRMamba/net_g_15000.pth"
echo "训练配置：options/train/finetune_IRSR_x4_bicubic_perturb_15000.yml"
export PYTHONPATH=/hpcfs/fhome/competitor01/lhx/IRSRMamba:$PYTHONPATH
cd /hpcfs/fhome/competitor01/lhx/IRSRMamba
python basicsr/train.py -opt options/train/finetune_IRSR_x4_bicubic_perturb_15000.yml
