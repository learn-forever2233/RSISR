...
#SBATCH --job-name=irsrmamba_x4_finetune # 作业名称
...
# 3. 执行微调训练任务
echo "开始微调训练 IRSRMamba 模型 (4倍超分辨率)，基于预训练模型 net_g_50000.pth"
python basicsr/train.py -opt options/train/finetune_IRSR_x4.yml
