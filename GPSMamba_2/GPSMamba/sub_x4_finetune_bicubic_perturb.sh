#!/bin/bash -l
#SBATCH -p qgpu_a800            # 指定分区 (这里使用 A800 GPU，有更大显存)
#SBATCH --job-name=ft_GPS_bicubic_perturb # 作业名称
...
# 3. 执行训练任务
echo "开始训练：使用参数扰动的 GPSMamba 双三次下采样训练"
echo "预训练模型：/hpcfs/fhome/competitor01/lhx/GPSMamba_2/GPSMamba/experiments/finetune_GPS_x4_mixed/models/net_g_25000.pth"
echo "训练配置：options/train/finetune_GPS_x4_bicubic_perturb.yml"
...
python basicsr/train.py -opt options/train/finetune_GPS_x4_bicubic_perturb.yml
